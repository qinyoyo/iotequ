...
<template>
  <div :class="joinMode?'cg-join-list':'cg-list'">
    <el-backtop v-if="isTableMode()" ref="cgListBacktop" class="el-backtop" target=".cg-list-peoplemapping .el-table__body-wrapper" :visibility-height="200">
    ...
    >
      ...
      <cg-table-column prop="readerNo" :page="paginationCurrentPage" :label="$t('devPeopleMapping.field.readerNo')" align="left" >
        <template slot-scope="scope">
          {{ scope.row.readerNo }}
        </template>

      </cg-table-column>
      <cg-table-column prop="userNo" :page="paginationCurrentPage" :label="$t('devPeopleMapping.field.userNo')" align="left" >
        <template slot-scope="scope">
          {{ scope.row.userNo }}
        </template>

      </cg-table-column>
      <cg-table-column prop="status" :page="paginationCurrentPage" :label="$t('devPeopleMapping.field.status')" align="left" >
        <template slot-scope="scope">
          {{ scope.row.status }}
        </template>
    ...
    <nut-scroller v-if="mobile && isTableMode()"
                  wrapperElement=".cg-list-peoplemapping .el-table__body-wrapper"
                  :isLoading="listLoading"
...
const Comp = {
  ...
  data() {
    return {
      ...
      queryRecordFields: [],
      formPath: '/reader/devPeopleMapping/record',
      idField: 'id',
      paginationCurrentPage: 1,
      paginationPageSize: this.$store.state.app.device === 'mobile' ? 10 : 30,
      paginationTotalRecords: 0,
      listName: 'peopleMapping',
...
const mixins = [Comp]
const mixinContext = require.context('.', false, /CgListPeopleMapping-mixin\.(js|vue)$/)
