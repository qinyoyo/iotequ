    ...
    <nut-actionsheet :is-visible="isVisible" @close="closeActionSheet">
        <div class="nut-picker" slot="custom" :class="customClassName ? customClassName : null">
            <div class="nut-picker-control">
                <span class="nut-picker-confirm-btn"  @click="closeActionSheet">{{$t('system.action.cancel')}}</span>
                <div class="nut-picker-title">{{title ? title : ''}}</div>
                <span class="nut-picker-cancel-btn" @click="confirm">{{$t('system.action.ok')}}</span>
            </div>
...
export default {
    ...
    watch: {
        ...
        isVisible(v) { // add by qinyoyo for ios bug
            if (v && this.$el) document.body.appendChild(this.$el);
            else if (!v && this.$el) document.body.removeChild(this.$el);
        }
    },
    methods: {
