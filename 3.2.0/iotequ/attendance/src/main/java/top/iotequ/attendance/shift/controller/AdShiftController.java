...
@RestController
@RequestMapping("/attendance/shift/adShift")
public class AdShiftController  {
	private static final Logger log = LoggerFactory.getLogger(AdShiftController.class);
	@Autowired
	ICgService<AdShift> cgService;
	...
	public ResponseEntity<Map<String, Object>> defaultValue(Boolean needLoadDictionary, String requestDynaFields, AdShift adShift,HttpServletRequest request, HttpServletResponse response) throws Exception {
		try {
			if (!Util.isEmpty(requestDynaFields)) return new RestJson().dictionary(cgService.getDictionary(adShift,true, requestDynaFields)).toResponse();
			else {
				adShift = cgService.getDefaultObject(adShift);
                return new RestJson().data(adShift)
                    .dictionary(Objects.nonNull(needLoadDictionary) && needLoadDictionary ? cgService.getDictionary(adShift,true,null) : null).toResponse();
			}
		} catch (Exception e) {
			return new RestJson().setError(e).toResponse();
		}
	...
	public ResponseEntity<Map<String, Object>> record(Boolean needLoadDictionary, Boolean loadDictionaryOnly, String requestDynaFields, String id, AdShift adShift, HttpServletRequest request, HttpServletResponse response) throws Exception {
		try {
			RestJson j = new RestJson();
			if (!Util.isEmpty(requestDynaFields) || (loadDictionaryOnly!=null && loadDictionaryOnly)) {
				return j.dictionary(cgService.getDictionary(adShift,true, requestDynaFields)).toResponse();
			}
			if (Objects.nonNull(needLoadDictionary) && needLoadDictionary) j.dictionary(cgService.getDictionary(adShift,true,null));
			if (!Util.isEmpty(id)) adShift=cgService.getRecord(id,request);
			j.data(adShift);
			return j.toResponse();
		} catch (Exception e) {
			return new RestJson().setError(e).toResponse();
	...
	public ResponseEntity<Map<String, Object>> save(String flowCode, Integer total_filepart, AdShift adShift, String idSaved, HttpServletRequest request, HttpServletResponse response) {
		try {
			boolean isNew = (Util.isEmpty(idSaved) || Util.isEmpty(adShift.getId()));
			return 	cgService.doSave(isNew, flowCode,total_filepart, adShift, idSaved, request).toResponse();
		} catch (Exception e) {
	...
	public ResponseEntity<Map<String, Object>> export(String sort,String order, String search,HttpServletRequest request, HttpServletResponse response){
		try {
			cgService.export(null,0,100, null, "adShift.xlsx", "adShift", sort, order, search, request, response);
			return null;
		} catch (Exception e) {
			response.reset();
