...
@RestController
@RequestMapping("/ewallet/ewDevice")
public class EwDeviceController  {
	private static final Logger log = LoggerFactory.getLogger(EwDeviceController.class);
	@Autowired
	ICgService<EwDevice> cgService;
	...
	public ResponseEntity<Map<String, Object>> defaultValue(Boolean needLoadDictionary, String requestDynaFields, EwDevice ewDevice,HttpServletRequest request, HttpServletResponse response) throws Exception {
		try {
			if (!Util.isEmpty(requestDynaFields)) return new RestJson().dictionary(cgService.getDictionary(ewDevice,true, requestDynaFields)).toResponse();
			else {
				ewDevice = cgService.getDefaultObject(ewDevice);
                return new RestJson().data(ewDevice)
                    .dictionary(Objects.nonNull(needLoadDictionary) && needLoadDictionary ? cgService.getDictionary(ewDevice,true,null) : null).toResponse();
			}
		} catch (Exception e) {
			return new RestJson().setError(e).toResponse();
		}
	...
	public ResponseEntity<Map<String, Object>> record(Boolean needLoadDictionary, Boolean loadDictionaryOnly, String requestDynaFields, String id, EwDevice ewDevice, HttpServletRequest request, HttpServletResponse response) throws Exception {
		try {
			RestJson j = new RestJson();
			if (!Util.isEmpty(requestDynaFields) || (loadDictionaryOnly!=null && loadDictionaryOnly)) {
				return j.dictionary(cgService.getDictionary(ewDevice,true, requestDynaFields)).toResponse();
			}
			if (Objects.nonNull(needLoadDictionary) && needLoadDictionary) j.dictionary(cgService.getDictionary(ewDevice,true,null));
			if (!Util.isEmpty(id)) ewDevice=cgService.getRecord(id,request);
			j.data(ewDevice);
			return j.toResponse();
		} catch (Exception e) {
			return new RestJson().setError(e).toResponse();
	...
	public ResponseEntity<Map<String, Object>> save(String flowCode, Integer total_filepart, EwDevice ewDevice, String idSaved, HttpServletRequest request, HttpServletResponse response) {
		try {
			boolean isNew = (Util.isEmpty(idSaved) || Util.isEmpty(ewDevice.getId()));
			return 	cgService.doSave(isNew, flowCode,total_filepart, ewDevice, idSaved, request).toResponse();
		} catch (Exception e) {
	...
	public ResponseEntity<Map<String, Object>> updateSelective(@RequestBody List<Map<String,Object>> ewDeviceList, HttpServletRequest request, HttpServletResponse response) {
		try {
			return cgService.updateSelective(ewDeviceList).toResponse();
		} catch (Exception e) {
			return new RestJson().setMessage(e).toResponse();
		}
	}
	...
	public ResponseEntity<Map<String, Object>> export(String sort,String order, String search,HttpServletRequest request, HttpServletResponse response){
		try {
			cgService.export(null,0,100, null, "ewDevice.xlsx", "ewDevice", sort, order, search, request, response);
			return null;
		} catch (Exception e) {
			response.reset();
