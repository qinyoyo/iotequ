...
@RestController
@RequestMapping("/ewallet/ewCountProject")
public class EwCountProjectController  {
	private static final Logger log = LoggerFactory.getLogger(EwCountProjectController.class);
	@Autowired
	ICgService<EwCountProject> cgService;
	...
	public ResponseEntity<Map<String, Object>> defaultValue(Boolean needLoadDictionary, String requestDynaFields, EwCountProject ewCountProject,HttpServletRequest request, HttpServletResponse response) throws Exception {
		try {
			if (!Util.isEmpty(requestDynaFields)) return new RestJson().dictionary(cgService.getDictionary(ewCountProject,true, requestDynaFields)).toResponse();
			else {
				ewCountProject = cgService.getDefaultObject(ewCountProject);
                return new RestJson().data(ewCountProject)
                    .dictionary(Objects.nonNull(needLoadDictionary) && needLoadDictionary ? cgService.getDictionary(ewCountProject,true,null) : null).toResponse();
			}
		} catch (Exception e) {
			return new RestJson().setError(e).toResponse();
		}
	...
	public ResponseEntity<Map<String, Object>> record(Boolean needLoadDictionary, Boolean loadDictionaryOnly, String requestDynaFields, String id, EwCountProject ewCountProject, HttpServletRequest request, HttpServletResponse response) throws Exception {
		try {
			RestJson j = new RestJson();
			if (!Util.isEmpty(requestDynaFields) || (loadDictionaryOnly!=null && loadDictionaryOnly)) {
				return j.dictionary(cgService.getDictionary(ewCountProject,true, requestDynaFields)).toResponse();
			}
			if (Objects.nonNull(needLoadDictionary) && needLoadDictionary) j.dictionary(cgService.getDictionary(ewCountProject,true,null));
			if (!Util.isEmpty(id)) ewCountProject=cgService.getRecord(id,request);
	...
	public ResponseEntity<Map<String, Object>> save(String flowCode, Integer total_filepart, EwCountProject ewCountProject, String idSaved, HttpServletRequest request, HttpServletResponse response) {
		try {
			boolean isNew = (Util.isEmpty(idSaved) || Util.isEmpty(ewCountProject.getId()));
			return 	cgService.doSave(isNew, flowCode,total_filepart, ewCountProject, idSaved, request).toResponse();
		} catch (Exception e) {
	...
	public ResponseEntity<Map<String, Object>> updateSelective(@RequestBody List<Map<String,Object>> ewCountProjectList, HttpServletRequest request, HttpServletResponse response) {
		try {
			return cgService.updateSelective(ewCountProjectList).toResponse();
		} catch (Exception e) {
			return new RestJson().setMessage(e).toResponse();
		}
	}
	...
	public ResponseEntity<Map<String, Object>> export(String sort,String order, String search,HttpServletRequest request, HttpServletResponse response){
		try {
			cgService.export(null,0,100, null, "ewCountProject.xlsx", "ewCountProject", sort, order, search, request, response);
			return null;
		} catch (Exception e) {
			response.reset();
