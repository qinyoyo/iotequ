...
@RestController
@RequestMapping("/codegenerator/cgFormField")
public class CgFormFieldController  {
	private static final Logger log = LoggerFactory.getLogger(CgFormFieldController.class);
	@Autowired
	ICgService<CgFormField> cgService;
	...
	public ResponseEntity<Map<String, Object>> defaultValue(Boolean needLoadDictionary, String requestDynaFields, CgFormField cgFormField,HttpServletRequest request, HttpServletResponse response) throws Exception {
		try {
			if (!Util.isEmpty(requestDynaFields)) return new RestJson().dictionary(cgService.getDictionary(cgFormField,true, requestDynaFields)).toResponse();
			else {
				cgFormField = cgService.getDefaultObject(cgFormField);
                return new RestJson().data(cgFormField)
                    .dictionary(Objects.nonNull(needLoadDictionary) && needLoadDictionary ? cgService.getDictionary(cgFormField,true,null) : null).toResponse();
			}
		} catch (Exception e) {
			return new RestJson().setError(e).toResponse();
		}
	...
	public ResponseEntity<Map<String, Object>> record(Boolean needLoadDictionary, Boolean loadDictionaryOnly, String requestDynaFields, String id, CgFormField cgFormField, HttpServletRequest request, HttpServletResponse response) throws Exception {
		try {
			RestJson j = new RestJson();
			if (!Util.isEmpty(requestDynaFields) || (loadDictionaryOnly!=null && loadDictionaryOnly)) {
				return j.dictionary(cgService.getDictionary(cgFormField,true, requestDynaFields)).toResponse();
			}
			if (Objects.nonNull(needLoadDictionary) && needLoadDictionary) j.dictionary(cgService.getDictionary(cgFormField,true,null));
			if (!Util.isEmpty(id)) cgFormField=cgService.getRecord(id,request);
			j.data(cgFormField);
			return j.toResponse();
		} catch (Exception e) {
			return new RestJson().setError(e).toResponse();
	...
	public ResponseEntity<Map<String, Object>> save(String flowCode, Integer total_filepart, CgFormField cgFormField, String idSaved, HttpServletRequest request, HttpServletResponse response) {
		try {
			boolean isNew = (Util.isEmpty(idSaved) || Util.isEmpty(cgFormField.getId()));
			return 	cgService.doSave(isNew, flowCode,total_filepart, cgFormField, idSaved, request).toResponse();
		} catch (Exception e) {
	...
	public ResponseEntity<Map<String, Object>> updateSelective(@RequestBody List<Map<String,Object>> cgFormFieldList, HttpServletRequest request, HttpServletResponse response) {
		try {
			return cgService.updateSelective(cgFormFieldList).toResponse();
		} catch (Exception e) {
			return new RestJson().setMessage(e).toResponse();
		}
	}
	...
	public ResponseEntity<Map<String, Object>> export(String sort,String order, String search,HttpServletRequest request, HttpServletResponse response){
		try {
			cgService.export(null,0,100, null, "cgFormField.xlsx", "cgFormField", sort, order, search, request, response);
			return null;
		} catch (Exception e) {
			response.reset();
