package top.iotequ.attendance.data.controller;
...
@RestController
@RequestMapping("/attendance/data/adData")
public class AdDataController  {
	private static final Logger log = LoggerFactory.getLogger(AdDataController.class);
	@Autowired
	ICgService<AdData> cgService;
	...
	public ResponseEntity<Map<String, Object>> export(String sort,String order, String search,HttpServletRequest request, HttpServletResponse response){
		try {
			cgService.export(null,0,100, null, "adData.xlsx", "adData", sort, order, search, request, response);
			return null;
		} catch (Exception e) {
			response.reset();
