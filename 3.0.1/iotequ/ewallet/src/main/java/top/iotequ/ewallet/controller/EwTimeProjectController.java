...
@RestController
@RequestMapping("/ewallet/ewTimeProject")
public class EwTimeProjectController  {
	private static final Logger log = LoggerFactory.getLogger(EwTimeProjectController.class);
	@Autowired
	ICgService<EwTimeProject> cgService;
	...
	public ResponseEntity<Map<String, Object>> defaultValue(Boolean needLoadDictionary, String requestDynaFields, EwTimeProject ewTimeProject,HttpServletRequest request, HttpServletResponse response) throws Exception {
		try {
			if (!Util.isEmpty(requestDynaFields)) return new RestJson().dictionary(cgService.getDictionary(ewTimeProject,true, requestDynaFields)).toResponse();
			else {
				ewTimeProject = cgService.getDefaultObject(ewTimeProject);
                return new RestJson().data(ewTimeProject)
                    .dictionary(Objects.nonNull(needLoadDictionary) && needLoadDictionary ? cgService.getDictionary(ewTimeProject,true,null) : null).toResponse();
			}
		} catch (Exception e) {
			return new RestJson().setError(e).toResponse();
		}
	...
	public ResponseEntity<Map<String, Object>> record(Boolean needLoadDictionary, Boolean loadDictionaryOnly, String requestDynaFields, String id, EwTimeProject ewTimeProject, HttpServletRequest request, HttpServletResponse response) throws Exception {
		try {
			RestJson j = new RestJson();
			if (!Util.isEmpty(requestDynaFields) || (loadDictionaryOnly!=null && loadDictionaryOnly)) {
				return j.dictionary(cgService.getDictionary(ewTimeProject,true, requestDynaFields)).toResponse();
			}
			if (Objects.nonNull(needLoadDictionary) && needLoadDictionary) j.dictionary(cgService.getDictionary(ewTimeProject,true,null));
			if (!Util.isEmpty(id)) ewTimeProject=cgService.getRecord(id,request);
	...
	public ResponseEntity<Map<String, Object>> save(String flowCode, Integer total_filepart, EwTimeProject ewTimeProject, String idSaved, HttpServletRequest request, HttpServletResponse response) {
		try {
			boolean isNew = (Util.isEmpty(idSaved) || Util.isEmpty(ewTimeProject.getId()));
			return 	cgService.doSave(isNew, flowCode,total_filepart, ewTimeProject, idSaved, request).toResponse();
		} catch (Exception e) {
	...
	public ResponseEntity<Map<String, Object>> export(String sort,String order, String search,HttpServletRequest request, HttpServletResponse response){
		try {
			cgService.export(null,0,100, null, "ewTimeProject.xlsx", "ewTimeProject", sort, order, search, request, response);
			return null;
		} catch (Exception e) {
			response.reset();
