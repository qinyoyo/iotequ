...
@RestController
@RequestMapping("/pay/payOperator")
public class PayOperatorController  {
	private static final Logger log = LoggerFactory.getLogger(PayOperatorController.class);
	@Autowired
	ICgService<PayOperator> cgService;
	...
	public ResponseEntity<Map<String, Object>> defaultValue(Boolean needLoadDictionary, String requestDynaFields, PayOperator payOperator,HttpServletRequest request, HttpServletResponse response) throws Exception {
		try {
			if (!Util.isEmpty(requestDynaFields)) return new RestJson().dictionary(cgService.getDictionary(payOperator,true, requestDynaFields)).toResponse();
			else {
				payOperator = cgService.getDefaultObject(payOperator);
                return new RestJson().data(payOperator)
                    .dictionary(Objects.nonNull(needLoadDictionary) && needLoadDictionary ? cgService.getDictionary(payOperator,true,null) : null).toResponse();
			}
		} catch (Exception e) {
			return new RestJson().setError(e).toResponse();
		}
	...
	public ResponseEntity<Map<String, Object>> record(Boolean needLoadDictionary, Boolean loadDictionaryOnly, String requestDynaFields, String id, PayOperator payOperator, HttpServletRequest request, HttpServletResponse response) throws Exception {
		try {
			RestJson j = new RestJson();
			if (!Util.isEmpty(requestDynaFields) || (loadDictionaryOnly!=null && loadDictionaryOnly)) {
				return j.dictionary(cgService.getDictionary(payOperator,true, requestDynaFields)).toResponse();
			}
			if (Objects.nonNull(needLoadDictionary) && needLoadDictionary) j.dictionary(cgService.getDictionary(payOperator,true,null));
			if (!Util.isEmpty(id)) payOperator=cgService.getRecord(id,request);
			j.data(payOperator);
			return j.toResponse();
		} catch (Exception e) {
			return new RestJson().setError(e).toResponse();
	...
	public ResponseEntity<Map<String, Object>> save(String flowCode, Integer total_filepart, PayOperator payOperator, String idSaved, HttpServletRequest request, HttpServletResponse response) {
		try {
			boolean isNew = (Util.isEmpty(idSaved) || Util.isEmpty(payOperator.getId()));
			return 	cgService.doSave(isNew, flowCode,total_filepart, payOperator, idSaved, request).toResponse();
		} catch (Exception e) {
	...
	public ResponseEntity<Map<String, Object>> export(String sort,String order, String search,HttpServletRequest request, HttpServletResponse response){
		try {
			cgService.export(null,0,100, null, "payOperator.xlsx", "payOperator", sort, order, search, request, response);
			return null;
		} catch (Exception e) {
			response.reset();
