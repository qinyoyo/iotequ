...
/*	@RequestMapping(value = D10ClientService.Callback_Record)
	...
	@RequestMapping(value = D10ClientService.Callback_Record)
	public void record(@RequestBody D10Record body) {
		log.debug("Callback_Record：{}",body);
		String dno=body.devnum;
		DevReader reader=readerDao.selectByReaderNo(dno);
		if (reader==null) log.info("设备不存在：{}",dno);
		else if (!isLicenceAuthorized(reader)) log.info("设备未获得Licence许可：{}",dno);
		else {
			Date dt= DateUtil.string2Date(body.date, "yyyyMMddHHmmss");
			if (dt!=null) {
				DeviceEvent event = new DeviceEvent(this);
				String uno=null;
				try { uno = body.userNum;	} catch (Exception e) {};
				Integer status=null;
				try { status = Integer.valueOf(body.status); } catch (Exception e) {}
				Integer deviceOrgCode=0;
				try {
					deviceOrgCode = SqlUtil.sqlQueryInteger(false, "select org_code from dev_reader_group where id=?", reader.getReaderGroup());
				} catch (Exception e) {	}
				Boolean warning = null;
				try { warning = Integer.valueOf(body.status)==2?true:false; } catch (Exception e) {}
				...
				event.put("status",status);
				event.put("orgCode", deviceOrgCode);
				event.put("authType", body.AuthType);
				event.put("auditeeAuthType", body.AuditeeAuthType);
				if(body.AuthType==1) {
					event.put("auditorUserNum", body.AuditorUserNum);
					Integer auditorOrg=0;
					try {
						auditorOrg=SqlUtil.sqlQueryInteger("select org_code from dev_people where user_no=?", body.AuditorUserNum);
					}catch (Exception e) {
					}
					event.put("auditorOrg", auditorOrg);
					event.put("auditorAuthType", body.AuditorAuthType);
				}
				applicationContext.publishEvent(event);
			} else log.error("error date format of {}",body.date);
		}
	}
