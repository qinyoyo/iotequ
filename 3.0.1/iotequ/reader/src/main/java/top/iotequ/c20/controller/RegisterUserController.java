...
public class RegisterUserController {
	...
	private DynaLog dynaLog = DynaLog.getLog(RegisterUserController.class);
	...
	public JSONObject BindingDevice(@RequestBody C20HttpServer.C20BindInfo bindInfo) {
		...
		Boolean status=true;
		String sql = "select * from dev_c20 where dev_no=? and sn_no=?";// 查询编号是否存在
		if (!SqlUtil.sqlExist(sql, bindInfo.devNo, bindInfo.SN)) {
			sql = "insert into dev_c20(sn_no,dev_no,is_allocated,date_time) values (?,?,false,?)";
			Date date = new Date();
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
			String createdate = sdf.format(date);
			try {
				SqlUtil.sqlExecute(sql, bindInfo.SN, bindInfo.devNo, createdate);
			} catch (Exception e) {
				status=false;
	...
	public JSONObject RegistUser(@RequestBody C20HttpServer.UserBaseInfo regUser) throws Exception {
		...
		dynaLog.e("REGISTER_USER:" + new Gson().toJson(regUser));
	...
	public JSONObject GetUserFinger(@RequestBody UserBaseInfoRsp rsp) throws Exception {
		...
		crm.status=true;
		crm.msg="上传成功";
		dynaLog.e("Upload User and Fingers:" + new Gson().toJson(rsp));
		// 直接注册用户 || 检查用户是否存在
	...
	public JSONObject delFinger(@RequestBody JSONObject body) throws Exception { // 日志
		...
		Boolean status=true;
		String msg = "删除成功";
		dynaLog.d("delMsg=" + body.toString());
	...
	public JSONObject UploadLog(@RequestBody C20LOG log) throws Exception { // 日志
		...
		Boolean status=true;
		String msg = "上传成功";
		dynaLog.d("C20 LogInfo" + new Gson().toJson(log));
	...
	public JSONObject AuthUserFinger(@RequestBody UploadAuthUser upload) throws Exception {
		...
		dynaLog.d("Auth Begin:" + new Gson().toJson(upload));
