...
//									EntityUtil.runMethod(bean,"removeTemplate", usernum,Integer.valueOf(f.getFignerIndex()));
	...
	private static List<DevAuthConfig> getAuthConfigBySql(String sql) {
		try {
			...
			List<DevAuthConfig> list = SqlUtil.sqlQuery(DevAuthConfig.class, false, "SELECT * FROM iotequ3.dev_auth_config where role_id = ? ",aa);
			return list;
		} catch (Exception e) {
			return null;
		}
		...
		try {
			...
			if (!Util.isEmpty(devGroup.getSubOrgAuth())) {
				...
				if (!Util.isEmpty(cc)) {
					for (Integer c: cc) {
						if (c==p.getOrgCode()) {
							authConfigs = SqlUtil.sqlQuery(DevAuthConfig.class, false, "SELECT * FROM iotequ3.dev_auth_config where role_id = ? ", devGroup.getSubOrgAuth());
							if (!Util.isEmpty(authConfigs)) authConfigList.addAll(authConfigs);
							break;
						}
					}
