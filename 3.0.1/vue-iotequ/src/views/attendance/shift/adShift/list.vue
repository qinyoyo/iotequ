...
export default {
  ...
  computed: {
    ...
    content() {
      if (this.showMaster || !this.mobile) return this.$t('adShift.title.list') + (!this.mobile && this.contentSubTitle?' - '+this.contentSubTitle:'')
      else return this.contentSubTitle ? this.contentSubTitle : this.$t('adShift.title.list')
    }
  },
  methods: {
    goBack() {
    ...
    setChildrenParams(row) {
      if (row && row.id) this.son0Condition.shiftId = row.id
      else this.son0Condition.shiftId = 'null'
      if (row && row.id) this.son1Condition.shiftId = row.id
      else this.son1Condition.shiftId = 'null'
    },
