...
<template>
  <div :class="joinMode?'cg-join-list':'cg-list'">
    <el-backtop v-if="isTableMode()" ref="cgListBacktop" class="el-backtop" target=".cg-list-adspecialshifttime .el-table__body-wrapper" :visibility-height="200">
    ...
    >
      ...
      <cg-table-column prop="name" type="dict" :page="1" :label="$t('adSpecialShiftTime.field.name')" align="left" >
        <template slot-scope="scope">
          {{ dictValue(scope.row.name,dictionary.dictName,false,true) }}
        </template>

      </cg-table-column>
      <cg-table-column prop="startTime" type="time" :page="1" :label="$t('adSpecialShiftTime.field.startTime')" align="center" >
        <template slot-scope="scope">
          {{ time2String(scope.row.startTime,'HH:mm') }}
        </template>

      </cg-table-column>
      <cg-table-column prop="endTime" type="time" :page="1" :label="$t('adSpecialShiftTime.field.endTime')" align="center" >
        <template slot-scope="scope">
...
const mixins = []
const mixinContext = require.context('.', false, /CgListAdSpecialShiftTime-mixin\.(js|vue)$/)
mixinContext.keys().forEach(key => { mixins.push(mixinContext(key).default) })
export default {
  name: 'CgListAdSpecialShiftTime',
  mixins,
  props: {
    joinMode: {
      type: Boolean,
  ...
  data() {
    return {
      ...
      rulesObject,
      path: 'list',
      title: this.$t('adSpecialShiftTime.title.list'),
      showQuery: false,
  ...
  computed: {
    ...
    className() {
      return this.openID ? 'cg-list-adspecialshifttime cg-list-adspecialshifttime' + '-'+this.openID : 'cg-list-adspecialshifttime'
    },
    allActions() {
