...
export default {
  ...
  computed: {
    ...
    content() {
      if (this.showMaster || !this.mobile) return this.$t('adSpecialShift.title.list') + (!this.mobile && this.contentSubTitle?' - '+this.contentSubTitle:'')
      else return this.contentSubTitle ? this.contentSubTitle : this.$t('adSpecialShift.title.list')
    }
  },
  methods: {
    goBack() {
    ...
    setChildrenParams(row) {
      ...
      if (row && row.id) this.son1Condition.specialShiftId = row.id
      else this.son1Condition.specialShiftId = 'null'
    },
