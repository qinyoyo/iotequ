    ...
    <el-card shadow="hover">
      ...
      <div v-if="mobile">
        <div v-show="showMaster" class="cg-father">
          <CgListPmProject ref="cgList" @detail="doShowDetail" @rowClick="rowClick" @refreshed="refreshed" :fixedQueryRecord="fixedQueryRecord" />
        </div>
      ...
      <div v-else class="components-container" :style="'height:'+clientHeight+'px'">
        <split-pane :default-percent="70" split="vertical">
          <template slot="paneL">
            <div class="cg-father">
              <CgListPmProject ref="cgList" @rowClick="rowClick" @refreshed="refreshed" :fixedQueryRecord="fixedQueryRecord" :height="fatherHeight" />
            </div>
          </template>
          <template slot="paneR">
...
export default {
  ...
  computed: {
    ...
    content() {
      if (this.showMaster || !this.mobile) return this.$t('pmProject.title.list')
      else return this.$t('pmProject.title.list')
    }
  },
  methods: {
    goBack() {
