...
<template>
  <div :class="joinMode?'cg-join-list':'cg-list'">
    <el-backtop v-if="isTableMode()" ref="cgListBacktop" class="el-backtop" target=".cg-list-pmpeoplegroup .el-table__body-wrapper" :visibility-height="200">
    ...
    >
      ...
      <el-table-column prop="name" width="200" :label="$t('pmPeopleGroup.field.name')" align="left" >
        <template slot-scope="scope">
          {{ scope.row.name }}
        </template>

      </el-table-column>
      <cg-table-column prop="groupType" type="dict" :page="1" :label="$t('pmPeopleGroup.field.groupType')" align="left" >
        <template slot-scope="scope">
          {{ dictValue(scope.row.groupType,dictionary.dictGroupType,true,true) }}
        </template>

      </cg-table-column>
      <cg-table-column prop="enabled" :page="1" :label="$t('pmPeopleGroup.field.enabled')" align="center" >
        <template slot-scope="scope">
          <cg-icon :color="scope.row.enabled?'#46a6ff':'grey'" :icon="scope.row.enabled?'fa fa-check-circle':'fa fa-circle-o'"/>
        </template>
...
const mixins = []
const mixinContext = require.context('.', false, /CgListPmPeopleGroup-mixin\.(js|vue)$/)
mixinContext.keys().forEach(key => { mixins.push(mixinContext(key).default) })
export default {
  name: 'CgListPmPeopleGroup',
  mixins,
  props: {
    joinMode: {
      type: Boolean,
  ...
  data() {
    return {
      ...
      myself: this,
      path: 'list',
      title: this.$t('pmPeopleGroup.title.list'),
      showQuery: false,
  ...
  computed: {
    ...
    className() {
      return this.openID ? 'cg-list-pmpeoplegroup cg-list-pmpeoplegroup' + '-'+this.openID : 'cg-list-pmpeoplegroup'
    },
    allActions() {
