...
export default  {
   ...
   getRules: function(obj, rec) {
      ...
      return Object.assign({
         name:[
            { required: true, message: vueObject.$t('system.message.needValue') + ':' + vueObject.$t('payOperator.field.name'), trigger: 'blur' }
         ],
         realName:[
            { required: true, message: vueObject.$t('system.message.needValue') + ':' + vueObject.$t('payOperator.field.realName'), trigger: 'blur' }
         ],
         password:[
            { required: true, message: vueObject.$t('system.message.needValue') + ':' + vueObject.$t('payOperator.field.password'), trigger: 'blur' }
         ],
         shopId:[
            { required: true, message: vueObject.$t('system.message.needValue') + ':' + vueObject.$t('payOperator.field.shopId'), trigger: 'blur' }
         ],
