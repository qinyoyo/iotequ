...
<template>
  <div :class="joinMode?'cg-join-list':'cg-list'">
    <el-backtop v-if="isTableMode()" ref="cgListBacktop" class="el-backtop" target=".cg-list-payoperator .el-table__body-wrapper" :visibility-height="200">
    ...
    >
      ...
      <cg-table-column prop="name" :page="paginationCurrentPage" :label="$t('payOperator.field.name')" align="left" >
        <template slot-scope="scope">
          {{ scope.row.name }}
        </template>

      </cg-table-column>
      <cg-table-column prop="realName" :page="paginationCurrentPage" :label="$t('payOperator.field.realName')" sortable align="left" >
        <template slot-scope="scope">
          {{ scope.row.realName }}
        </template>

      </cg-table-column>
      <cg-table-column prop="shopId" type="dict" :page="paginationCurrentPage" :label="$t('payOperator.field.shopId')" align="left" >
        <template slot-scope="scope">
          {{ dictValue(scope.row.shopId,dictionary.dictShopId,false,true) }}
        </template>

      </cg-table-column>
      <cg-table-column prop="userNo" :page="paginationCurrentPage" :label="$t('payOperator.field.userNo')" align="left" >
        <template slot-scope="scope">
          {{ scope.row.userNo }}
        </template>
    ...
    <cg-card-list v-else ref="cgList" v-loading="listLoading" :render="rowRender" :data="rows" :mainClass="className" row-key="id" :multiple="false" :height="tableHeight()" 
                  :isLoading="listLoading" :groupBy="rowRenderGroupTitle?rowRenderGroupTitle:'shopId'"
    ...
    <nut-scroller v-if="mobile && isTableMode()"
                  wrapperElement=".cg-list-payoperator .el-table__body-wrapper"
    ...
    <cg-query-condition v-model="showQuery" :modal="!joinMode" :queryRecord="queryRecord"
      ...
      <el-form-item v-show="queryRecord.search" :label="$t('system.action.field')" prop="searchFields" :size="$store.state.app.size">
        <cg-select v-model="queryRecord.searchFields" dictionary="realName|payOperator.field.realName," multiple/>
      </el-form-item>
      <el-divider />
      <div v-show="!queryRecord.search">
        <el-form-item :label="$t('payOperator.field.realName')" prop="realName" :size="$store.state.app.size">
          <el-input v-model="queryRecord.realName" type="text" name="realName"
                    :readonly="fixedQueryRecord.realName?true:false" :label="$t('payOperator.field.realName')" clearable resize autofocus/>
        </el-form-item>
      </div>
    </cg-query-condition>
  </div>
...
const mixins = []
const mixinContext = require.context('.', false, /CgListPayOperator-mixin\.(js|vue)$/)
mixinContext.keys().forEach(key => { mixins.push(mixinContext(key).default) })
export default {
  name: 'CgListPayOperator',
  mixins,
  props: {
    joinMode: {
      type: Boolean,
  ...
  data() {
    return {
      ...
      myself: this,
      path: 'list',
      title: this.$t('payOperator.title.list'),
      showQuery: false,
      showActionView: false,
      defaultOrder: 'id',
      queryRecord: this.initialQueryRecord(),
      queryRecordFields: ['realName'],
      formPath: '/pay/payOperator/record',
      listLoading: false,
      rows: [],
  ...
  computed: {
    ...
    className() {
      return this.openID ? 'cg-list-payoperator cg-list-payoperator' + '-'+this.openID : 'cg-list-payoperator'
    },
    allActions() {
