...
<template>
  <div :class="joinMode?'cg-join-list':'cg-list'">
    <el-backtop v-if="isTableMode()" ref="cgListBacktop" class="el-backtop" target=".cg-list-paypos .el-table__body-wrapper" :visibility-height="200">
    ...
    >
      ...
      <cg-table-column prop="no" :page="paginationCurrentPage" :label="$t('payPos.field.no')" sortable align="left" >
        <template slot-scope="scope">
          {{ scope.row.no }}
        </template>

      </cg-table-column>
      <cg-table-column prop="shopId" type="dict" :page="paginationCurrentPage" :label="$t('payPos.field.shopId')" align="left" >
        <template slot-scope="scope">
          {{ dictValue(scope.row.shopId,dictionary.dictShopId,false,true) }}
        </template>

      </cg-table-column>
      <cg-table-column prop="loginId" :page="paginationCurrentPage" :label="$t('payPos.field.loginId')" align="right" >
        <template slot-scope="scope">
          {{ scope.row.loginId }}
        </template>

      </cg-table-column>
      <cg-table-column prop="ewalletActive" :page="paginationCurrentPage" :label="$t('payPos.field.ewalletActive')" align="center" >
        <template slot-scope="scope">
          <cg-icon :color="scope.row.ewalletActive?'#46a6ff':'grey'" :icon="scope.row.ewalletActive?'fa fa-check-circle':'fa fa-circle-o'"/>
        </template>

      </cg-table-column>
      <cg-table-column prop="countProjectList" type="dict" :page="paginationCurrentPage" :label="$t('payPos.field.countProjectList')" align="left" >
        <template slot-scope="scope">
          {{ dictValue(scope.row.countProjectList,dictionary.dictCountProjectList,false,true) }}
        </template>

      </cg-table-column>
      <cg-table-column prop="timeProjectList" type="dict" :page="paginationCurrentPage" :label="$t('payPos.field.timeProjectList')" align="left" >
        <template slot-scope="scope">
          {{ dictValue(scope.row.timeProjectList,dictionary.dictTimeProjectList,false,true) }}
        </template>
    ...
    <nut-scroller v-if="mobile && isTableMode()"
                  wrapperElement=".cg-list-paypos .el-table__body-wrapper"
                  :isLoading="listLoading"
    ...
    <cg-query-condition v-model="showQuery" :modal="!joinMode" :queryRecord="queryRecord"
      ...
      <el-form-item v-show="queryRecord.search" :label="$t('system.action.field')" prop="searchFields" :size="$store.state.app.size">
        <cg-select v-model="queryRecord.searchFields" dictionary="no|payPos.field.no," multiple/>
      </el-form-item>
      <el-divider />
      <div v-show="!queryRecord.search">
        <el-form-item :label="$t('payPos.field.no')" prop="no" :size="$store.state.app.size">
          <el-input v-model="queryRecord.no" type="text" name="no"
                    :readonly="fixedQueryRecord.no?true:false" :label="$t('payPos.field.no')" clearable resize autofocus/>
        </el-form-item>
      </div>
    </cg-query-condition>
  </div>
...
const mixins = []
const mixinContext = require.context('.', false, /CgListPayPos-mixin\.(js|vue)$/)
...
export default {
  ...
  data() {
    return {
      ...
      myself: this,
      path: 'list',
      title: this.$t('payPos.title.list'),
      showQuery: false,
      showActionView: false,
      defaultOrder: 'shop_id,id',
      queryRecord: this.initialQueryRecord(),
      queryRecordFields: ['no'],
      formPath: '/pay/payPos/record',
      listLoading: false,
      rows: [],
  ...
  computed: {
    ...
    className() {
      return this.openID ? 'cg-list-paypos cg-list-paypos' + '-'+this.openID : 'cg-list-paypos'
    },
    allActions() {
