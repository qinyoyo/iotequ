<template>
  <div class="cg-list">
    <el-card shadow="hover">
      ...
      <CgListCgProject ref="cgList" :height="clientHeight" @detail="doShowDetail" :fixedQueryRecord="fixedQueryRecord" @rowClick="rowClick"/>
    </el-card>
  </div>
</template>
...
import CgListCgProject from './CgListCgProject.vue'
const mixins = []
...
export default {
  ...
  computed: {
    ...
    content() {
      return this.$t('cgProject.title.list') + (!this.mobile && this.contentSubTitle?' - '+this.contentSubTitle:'')
    }
  },
  methods: {
    goBack() {
