...
export default {
  ...
  computed: {
    ...
    content() {
      if (this.showMaster || !this.mobile) return this.$t('cgForm.title.list') + (!this.mobile && this.contentSubTitle?' - '+this.contentSubTitle:'')
      else return this.contentSubTitle ? this.contentSubTitle : this.$t('cgForm.title.list')
    }
  },
  methods: {
    goBack() {
    ...
    setChildrenParams(row) {
      ...
      if (row && row.id) this.son1Condition.formId = row.id
      else this.son1Condition.formId = 'null'
    },
