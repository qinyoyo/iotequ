...
export default  {
   ...
   getRules: function(obj, rec) {
      ...
      return Object.assign({
         name:[
            { required: true, message: vueObject.$t('system.message.needValue') + ':' + vueObject.$t('devAuthRole.field.name'), trigger: 'blur' },
            {
                validator: (rule, value, callback) => {
                  if (!value) callback()
                  else {
                    if (!/^(\S.*?\S|\S)$/.test(value)) callback(new Error(vueObject.$t(vueObject.$te('devAuthRole.field.nameValid') ? 'devAuthRole.field.nameValid' : 'system.message.errorValue')))
                    else callback()
                  }
                },
                trigger: 'blur'
