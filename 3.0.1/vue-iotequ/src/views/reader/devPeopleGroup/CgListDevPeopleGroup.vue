...
<template>
  <div :class="joinMode?'cg-join-list':'cg-list'">
    <el-backtop v-if="isTableMode()" ref="cgListBacktop" class="el-backtop" target=".cg-list-devpeoplegroup .el-table__body-wrapper" :visibility-height="200">
    ...
    >
      ...
      <cg-table-column prop="orgCode" type="dict" :page="paginationCurrentPage" :label="$t('devPeople.field.orgCode')" align="left" >
        <template slot-scope="scope">
    ...
    <nut-scroller v-if="mobile && isTableMode()"
                  wrapperElement=".cg-list-devpeoplegroup .el-table__body-wrapper"
                  :isLoading="listLoading"
...
const mixins = []
const mixinContext = require.context('.', false, /CgListDevPeopleGroup-mixin\.(js|vue)$/)
mixinContext.keys().forEach(key => { mixins.push(mixinContext(key).default) })
export default {
  name: 'CgListDevPeopleGroup',
  mixins,
  props: {
    joinMode: {
      type: Boolean,
  ...
  data() {
    return {
      ...
      myself: this,
      path: 'list',
      title: this.$t('devPeopleGroup.title.list'),
      showQuery: false,
  ...
  computed: {
    ...
    className() {
      return this.openID ? 'cg-list-devpeoplegroup cg-list-devpeoplegroup' + '-'+this.openID : 'cg-list-devpeoplegroup'
    },
    allActions() {
      if (this.joinMode) return 'refresh'
      else return ',list,delete,batdel,picker,add,'
    }
  },
  watch: {
    fixedQueryRecord: {
