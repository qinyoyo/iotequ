    ...
    <el-form ref="cgForm" v-loading="recordLoading" :model="record" v-cg-form-adjust
      ...
      <cg-join v-model="userNoJoinVisible" :readonly="isDetail" >
        <CgListDevPeopleSelect slot="popover" ref="userNoJoin" openID="userno-join" :height="500" :joinShow="userNoJoinVisible" :joinMultiple="true"
                               :originSelections="record.userNo" selectionKey="userNo" joinMode @closeJoinList="(rows)=>{ getJoinFields('userNo',rows)}"
...
export default {
  ...
  data() {
    return {
      myself: this,
      path: 'record',
      title: this.$t('devPeopleGroup.title.'+this.path),
      rules: {},
      idField: 'id',
  ...
  computed: {
    ...
    className() {
      return (this.mobile?'cg-form-cell ':'')+'cg-no-border cg-form-devpeoplegroup'
    },
    labelWidth() {
