...
<template>
  <div :class="joinMode?'cg-join-list':'cg-list'">
    <el-backtop v-if="isTableMode()" ref="cgListBacktop" class="el-backtop" target=".cg-list-downloadplan .el-table__body-wrapper" :visibility-height="200">
    ...
    >
      ...
      <cg-table-column prop="userNo" :page="paginationCurrentPage" :label="$t('devDownloadPlan.field.userNo')" align="left" >
        <template slot-scope="scope">
          {{ scope.row.userNo }}
        </template>

      </cg-table-column>
      <cg-table-column prop="readerNo" :page="paginationCurrentPage" :label="$t('devDownloadPlan.field.readerNo')" align="left" >
        <template slot-scope="scope">
          {{ scope.row.readerNo }}
        </template>

      </cg-table-column>
      <cg-table-column prop="type" :page="paginationCurrentPage" :label="$t('devDownloadPlan.field.type')" align="right" >
        <template slot-scope="scope">
          {{ scope.row.type }}
        </template>

      </cg-table-column>
      <cg-table-column prop="downloadNum" :page="paginationCurrentPage" :label="$t('devDownloadPlan.field.downloadNum')" align="right" >
        <template slot-scope="scope">
          {{ scope.row.downloadNum }}
        </template>

      </cg-table-column>
      <cg-table-column prop="time" type="datetime" :page="paginationCurrentPage" :label="$t('devDownloadPlan.field.time')" align="left" >
        <template slot-scope="scope">
    ...
    <nut-scroller v-if="mobile && isTableMode()"
                  wrapperElement=".cg-list-downloadplan .el-table__body-wrapper"
                  :isLoading="listLoading"
...
const Comp = {
  ...
  data() {
    return {
      ...
      queryRecordFields: [],
      formPath: '/reader/devDownloadPlan/record',
      idField: 'id',
      paginationCurrentPage: 1,
      paginationPageSize: this.$store.state.app.device === 'mobile' ? 10 : 30,
      paginationTotalRecords: 0,
      listName: 'downloadPlan',
...
const mixins = [Comp]
const mixinContext = require.context('.', false, /CgListDownloadPlan-mixin\.(js|vue)$/)
