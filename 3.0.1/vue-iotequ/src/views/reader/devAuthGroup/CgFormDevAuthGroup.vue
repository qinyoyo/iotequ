...
<template>
  <div :class="mobile || height==0 ? 'cg-form':'cg-form just-v-scroll'" :style="mobile || height==0?'':'height:'+height+'px'">
    <el-form ref="cgForm" v-loading="recordLoading" :model="record" v-cg-form-adjust
      ...
      <el-form-item class="cg-item-mltext cg-auto-focus" :label="$t('devAuthGroup.field.name')" prop="name" :size="$store.state.app.size" >
      ...
      <el-form-item class="cg-item-select" :label="$t('devAuthGroup.field.auth')" prop="auth" :size="$store.state.app.size" >
        <cg-select v-model="record.auth" name="auth"
                   :dictionary="dictionary.dictAuth" :readonly="isDetail" :filterable="false" :allow-create="false" multiple :placeholder="$t('system.message.needValue')" />
      </el-form-item>
    </el-form>
...
const mixins = [Comp]
const mixinContext = require.context('.', false, /CgFormDevAuthGroup-mixin\.(js|vue)$/)
