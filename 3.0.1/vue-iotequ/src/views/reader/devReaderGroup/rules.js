...
export default  {
   ...
   getRules: function(obj, rec) {
      ...
      return Object.assign({
         name:[
            { required: true, message: vueObject.$t('system.message.needValue') + ':' + vueObject.$t('devReaderGroup.field.name'), trigger: 'blur' },
            {
                validator: (rule, value, callback) => {
                    if (!value) callback()
                    else {
                       if (!/^(\S.*?\S|\S)$/.test(value)) callback(new Error(vueObject.$t(vueObject.$te('devReaderGroup.field.nameValid') ? 'devReaderGroup.field.nameValid' : 'system.message.errorValue')))
                       else callback()
                    }
                },
                trigger: 'blur'
         ...
         orgCode:[
            { required: true, message: vueObject.$t('system.message.needValue') + ':' + vueObject.$t('devReaderGroup.field.orgCode'), trigger: 'blur' }
         ],
