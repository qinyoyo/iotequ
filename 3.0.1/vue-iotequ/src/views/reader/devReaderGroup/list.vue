...
export default {
  ...
  computed: {
    ...
    content() {
      if (this.showMaster || !this.mobile) return this.$t('devReaderGroup.title.list') + (!this.mobile && this.contentSubTitle?' - '+this.contentSubTitle:'')
      else return this.contentSubTitle ? this.contentSubTitle : this.$t('devReaderGroup.title.list')
    }
  },
  methods: {
    goBack() {
    ...
    setChildrenParams(row) {
      if (row && row.id) this.son0Condition.readerGroup = row.id
      else this.son0Condition.readerGroup = 'null'
    },
