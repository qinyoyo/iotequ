...
<template>
  <div :class="joinMode?'cg-join-list':'cg-list'">
    <el-backtop v-if="isTableMode()" ref="cgListBacktop" class="el-backtop" target=".cg-list-devreadergroup .el-table__body-wrapper" :visibility-height="200">
    ...
    >
      ...
      <el-table-column prop="name" width="300" :label="$t('devReaderGroup.field.name')" align="left" >
        <template slot-scope="scope">
          {{ scope.row.name }}
        </template>

      </el-table-column>
      <cg-table-column prop="orgCode" type="dict" :page="1" :label="$t('devReaderGroup.field.orgCode')" align="left" >
        <template slot-scope="scope">
          {{ dictValue(scope.row.orgCode,dictionary.dictOrgCode,true,true) }}
        </template>

      </cg-table-column>
      <cg-table-column prop="authGroupList" type="dict" :page="1" :label="$t('devReaderGroup.field.authGroupList')" align="left" >
        <template slot-scope="scope">
          {{ dictValue(scope.row.authGroupList,dictionary.dictAuthGroupList,false,true) }}
        </template>

      </cg-table-column>
      <cg-table-column prop="orgAuth" type="dict" :page="1" :label="$t('devReaderGroup.field.orgAuth')" align="left" >
        <template slot-scope="scope">
          {{ dictValue(scope.row.orgAuth,dictionary.dictOrgAuth,false,true) }}
        </template>

      </cg-table-column>
      <cg-table-column prop="subOrgAuth" type="dict" :page="1" :label="$t('devReaderGroup.field.subOrgAuth')" align="left" >
        <template slot-scope="scope">
          {{ dictValue(scope.row.subOrgAuth,dictionary.dictSubOrgAuth,false,true) }}
        </template>
...
const mixins = []
const mixinContext = require.context('.', false, /CgListDevReaderGroup-mixin\.(js|vue)$/)
mixinContext.keys().forEach(key => { mixins.push(mixinContext(key).default) })
export default {
  name: 'CgListDevReaderGroup',
  mixins,
  props: {
    joinMode: {
      type: Boolean,
  ...
  data() {
    return {
      ...
      myself: this,
      path: 'list',
      title: this.$t('devReaderGroup.title.list'),
      showQuery: false,
  ...
  computed: {
    ...
    className() {
      return this.openID ? 'cg-list-devreadergroup cg-list-devreadergroup' + '-'+this.openID : 'cg-list-devreadergroup'
    },
    allActions() {
      if (this.joinMode) return 'refresh'
      else return ',list,add,edit,delete,import,export,'
    }
  },
  watch: {
    fixedQueryRecord: {
