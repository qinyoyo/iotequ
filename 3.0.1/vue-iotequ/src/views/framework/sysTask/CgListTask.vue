...
<template>
  <div :class="joinMode?'cg-join-list':'cg-list'">
    <el-backtop v-if="isTableMode()" ref="cgListBacktop" class="el-backtop" target=".cg-list-task .el-table__body-wrapper" :visibility-height="200">
    ...
    >
      ...
      <cg-table-column prop="run" :page="1" :label="$t('sysTask.field.run')" align="left" >
        <template slot-scope="scope">
          {{ scope.row.run }}
        </template>

      </cg-table-column>
      <cg-table-column prop="name" :page="1" :label="$t('sysTask.field.name')" align="left" >
        <template slot-scope="scope">
          {{ scope.row.name }}
        </template>

      </cg-table-column>
      <cg-table-column prop="description" :page="1" :label="$t('sysTask.field.description')" align="left" >
        <template slot-scope="scope">
          {{ scope.row.description }}
        </template>

      </cg-table-column>
      <cg-table-column prop="className" :page="1" :label="$t('sysTask.field.className')" align="left" >
        <template slot-scope="scope">
          {{ scope.row.className }}
        </template>

      </cg-table-column>
      <cg-table-column prop="mothodName" :page="1" :label="$t('sysTask.field.mothodName')" align="left" >
        <template slot-scope="scope">
          {{ scope.row.mothodName }}
        </template>

      </cg-table-column>
      <cg-table-column prop="isRunning" :page="1" :label="$t('sysTask.field.isRunning')" align="center" >
        <template slot-scope="scope">
...
const mixins = []
const mixinContext = require.context('.', false, /CgListTask-mixin\.(js|vue)$/)
...
export default {
  ...
  data() {
    return {
      ...
      myself: this,
      path: 'list',
      title: this.$t('sysTask.title.list'),
      showQuery: false,
  ...
  computed: {
    ...
    className() {
      return this.openID ? 'cg-list-task cg-list-task' + '-'+this.openID : 'cg-list-task'
    },
    allActions() {
