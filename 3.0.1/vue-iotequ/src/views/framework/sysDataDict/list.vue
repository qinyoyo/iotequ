<template>
  <div class="cg-list">
    <el-card shadow="hover">
      ...
      <CgListDataDict ref="cgList" :height="clientHeight" @detail="doShowDetail" @rowClick="rowClick"/>
    </el-card>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    content() {
      return this.$t('sysDataDict.title.list') + (!this.mobile && this.contentSubTitle?' - '+this.contentSubTitle:'')
    }
  },
  methods: {
    goBack() {
    ...
    rowClick({ row, column, event }) {
      this.contentSubTitle = row && row.text ? String(row.text).local() : ''
    },
