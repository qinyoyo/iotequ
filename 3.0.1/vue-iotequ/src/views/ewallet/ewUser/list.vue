...
export default {
  ...
  computed: {
    ...
    content() {
      if (this.showMaster || !this.mobile) return this.$t('ewUser.title.list') + (!this.mobile && this.contentSubTitle?' - '+this.contentSubTitle:'')
      else return this.contentSubTitle ? this.contentSubTitle : this.$t('ewUser.title.list')
    }
  },
  methods: {
    goBack() {
    ...
    setChildrenParams(row) {
      if (row && row.userNo) this.son0Condition.userNo = row.userNo
      else this.son0Condition.userNo = 'null'
      if (row && row.userNo) this.son1Condition.userNo = row.userNo
      else this.son1Condition.userNo = 'null'
    },
