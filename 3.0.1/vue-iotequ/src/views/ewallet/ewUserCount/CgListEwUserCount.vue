...
<template>
  <div :class="joinMode?'cg-join-list':'cg-list'">
    <el-backtop v-if="isTableMode()" ref="cgListBacktop" class="el-backtop" target=".cg-list-ewusercount .el-table__body-wrapper" :visibility-height="200">
    ...
    >
      ...
      <cg-table-column prop="userNo" :page="paginationCurrentPage" :label="$t('ewUserCount.field.userNo')" sortable align="left" >
        <template slot-scope="scope">
          {{ scope.row.userNo }}
        </template>

      </cg-table-column>
      <cg-table-column prop="countId" :page="paginationCurrentPage" :label="$t('ewUserCount.field.countId')" align="left" >
        <template slot-scope="scope">
          {{ scope.row.countId }}
        </template>

      </cg-table-column>
      <cg-table-column prop="amountCount" :page="paginationCurrentPage" :label="$t('ewUserCount.field.amountCount')" align="right" >
        <template slot-scope="scope">
          {{ scope.row.amountCount }}
        </template>

      </cg-table-column>
      <cg-table-column prop="costLimit" :page="paginationCurrentPage" :label="$t('ewUserCount.field.costLimit')" align="right" >
        <template slot-scope="scope">
          {{ scope.row.costLimit }}
        </template>

      </cg-table-column>
      <cg-table-column prop="dayLimit" :page="paginationCurrentPage" :label="$t('ewUserCount.field.dayLimit')" align="right" >
        <template slot-scope="scope">
          {{ scope.row.dayLimit }}
        </template>
    ...
    <nut-scroller v-if="mobile && isTableMode()"
                  wrapperElement=".cg-list-ewusercount .el-table__body-wrapper"
                  :isLoading="listLoading"
    ...
    <cg-query-condition v-model="showQuery" :modal="!joinMode" :queryRecord="queryRecord"
      ...
      <el-form-item v-show="queryRecord.search" :label="$t('system.action.field')" prop="searchFields" :size="$store.state.app.size">
        <cg-select v-model="queryRecord.searchFields" dictionary="userNo|ewUserCount.field.userNo," multiple/>
      </el-form-item>
      <el-divider />
...
const mixins = []
const mixinContext = require.context('.', false, /CgListEwUserCount-mixin\.(js|vue)$/)
mixinContext.keys().forEach(key => { mixins.push(mixinContext(key).default) })
export default {
  name: 'CgListEwUserCount',
  mixins,
  props: {
    joinMode: {
      type: Boolean,
  ...
  data() {
    return {
      ...
      myself: this,
      path: 'list',
      title: this.$t('ewUserCount.title.list'),
      showQuery: false,
  ...
  computed: {
    ...
    className() {
      return this.openID ? 'cg-list-ewusercount cg-list-ewusercount' + '-'+this.openID : 'cg-list-ewusercount'
    },
    allActions() {
