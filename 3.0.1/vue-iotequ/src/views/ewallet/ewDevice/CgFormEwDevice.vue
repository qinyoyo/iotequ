...
<template>
  <div :class="mobile || height==0 ? 'cg-form':'cg-form just-v-scroll'" :style="mobile || height==0?'':'height:'+height+'px'">
    <el-form ref="cgForm" v-loading="recordLoading" :model="record" v-cg-form-adjust
      ...
      <el-form-item class="cg-item-checkbox" :label="$t('ewDevice.field.privilegeList')" prop="privilegeList" :size="$store.state.app.size" >
        <cg-checkbox v-model="record.privilegeList" name="privilegeList" :dictionary="dictionary.dictPrivilegeList" :readonly="isDetail"  />
      </el-form-item>
    </el-form>
...
const mixins = []
const mixinContext = require.context('.', false, /CgFormEwDevice-mixin\.(js|vue)$/)
...
export default {
  ...
  data() {
    return {
      myself: this,
      path: 'record',
      title: this.$t('ewDevice.title.'+this.path),
      rules: {},
      idField: 'id',
  ...
  computed: {
    ...
    className() {
      return (this.mobile?'cg-form-cell ':'')+'cg-no-border cg-form-ewdevice'
    },
    labelWidth() {
