<template>
  <div class="cg-list">
    <el-card shadow="hover">
      ...
      <CgListAuthConfig ref="cgList" :height="clientHeight" @detail="doShowDetail"/>
    </el-card>
  </div>
</template>
...
import CgListAuthConfig from './CgListAuthConfig.vue'
const mixins = []
const mixinContext = require.context('.', false, /list-mixin\.(js|vue)$/)
mixinContext.keys().forEach(key => { mixins.push(mixinContext(key).default) })
export default {
  name: 'AuthConfigList',
  components: { CgListAuthConfig },
  mixins,
  data() {
    return {
      clientHeight: this.height ? this.height : cg.containerHeight(),
      fatherHeight: (this.height ? this.height : cg.containerHeight()) * (this.$store.state.app.device === 'mobile' ? 1: 0 / 100),
      childHeight: (this.height?this.height:cg.containerHeight())*(this.$store.state.app.device === 'mobile' ? 1 : (100- 0)/100)-70,
      path: 'list',
  ...
  computed: {
    ...
    content() {
      return this.$t('devAuthConfig.title.list')
    }
  },
  methods: {
    goBack() {
