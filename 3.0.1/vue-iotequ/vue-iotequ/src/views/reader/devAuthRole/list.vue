...
export default {
  ...
  computed: {
    ...
    content() {
      if (this.showMaster || !this.mobile) return this.$t('devAuthRole.title.list')
      else return this.$t('devAuthRole.title.list')
    }
  },
  methods: {
    goBack() {
    ...
    setChildrenParams(row) {
      if (row && row.id) this.son0Condition.roleId = row.id
      else this.son0Condition.roleId = 'null'
    },
